...
def run_single_experiment(config, seed):
    ...
    args.attack_type = config.get('attack_type', 'sign_flip')
        ...
        for sim_name, sim_mode, sim_params in simulations:
            for alg_name, alg_id, alg_params in algorithms:
                config = {
                    ...
                    'optimizer': 'adam',
                    'metrics': ['accuracy'],
                    'skew_alpha': sim_params.get('skew_alpha', 0.5),  # Ensure skew_alpha is always set
                    'dirichlet_alpha': sim_params.get('dirichlet_alpha', 0.5),  # Default dirichlet_alpha
                    **alg_params,
                }
...
def format_byzantine_results(results):
    ...
    for result in results:
        cfg = result['config']
        attack = 'No Attack' if cfg['attack_type'] == 'none' else f"f={len(cfg['byzantine_ids'])} {cfg['attack_type']}"
