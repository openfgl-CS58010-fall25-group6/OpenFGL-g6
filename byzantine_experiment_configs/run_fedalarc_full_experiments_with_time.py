...
def format_byzantine_results(results):
    """Format results as a pivot table showing last round accuracy and timing"""
    df_data = []
...
def main():
    ...
    table_acc, table_time = format_byzantine_results(results)
    print("\n" + "="*70)
    ...
    print("\n" + "="*70)
    print("TIMING (seconds per experiment, averaged over seeds)")
    print("="*70)
    ...
    print(f"Accuracy table saved to: {csv_file}")
    
    csv_time_file = f'results/fedalarc_time_{args.dataset}_{timestamp}.csv'
    table_time.to_csv(csv_time_file)
    print(f"Timing table saved to: {csv_time_file}")
